...
import { maxYearIn, pairCounter, sumCounter } from '@/composables/useCounter'
...
describe('sumCounter', () => {
  ...
  it('returns the scalar as-is on legacy data regardless of period', () => {
    ...
    expect(sumCounter(42, { kind: 'thisYear' }, updatedYear)).toBe(42)
  ...
  it('returns byYear[updatedYear] on thisYear', () => {
    const c = { total: 100, byYear: { 2026: 10, 2025: 90 } }
    expect(sumCounter(c, { kind: 'thisYear' }, updatedYear)).toBe(10)
  })

  it('returns byYear[updatedYear - 1] on lastYear', () => {
    const c = { total: 100, byYear: { 2026: 10, 2025: 90 } }
    expect(sumCounter(c, { kind: 'lastYear' }, updatedYear)).toBe(90)
  })

  it('returns 0 on thisYear when the reference year is absent from byYear', () => {
    const c = { total: 100, byYear: { 2025: 100 } }
    expect(sumCounter(c, { kind: 'thisYear' }, updatedYear)).toBe(0)
  })
...
describe('maxYearIn', () => {
  it('returns undefined when all inputs are undefined or empty', () => {
    expect(maxYearIn()).toBeUndefined()
    expect(maxYearIn(undefined, {})).toBeUndefined()
  })

  it('returns the max year across the provided maps', () => {
    expect(maxYearIn({ 2020: 1 }, { 2024: 2, 2019: 3 })).toBe(2024)
  })

  it('skips non-numeric keys', () => {
    expect(maxYearIn({ 2024: 1, foo: 5 } as unknown as Record<string, number>)).toBe(2024)
  })
})
